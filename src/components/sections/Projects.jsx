        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-32 gap-8">
          ...
          >
            ...
            <p className="mt-8 text-slate-500 dark:text-slate-400 font-medium tracking-wide max-w-md">
               Crafting production-grade digital experiences with focus on architecture, scalability, and performance.
            </p>
          </motion.div>
      ...
      <div className="relative overflow-hidden rounded-[4rem] border border-white/40 dark:border-slate-800/50 bg-gradient-to-br from-white/60 via-slate-50/40 to-white/60 dark:from-slate-900/60 dark:via-[#020617]/40 dark:to-slate-900/60 backdrop-blur-3xl p-8 md:p-20 shadow-[0_40px_100px_-20px_rgba(0,0,0,0.05)] transition-all duration-700 group-hover:border-orange-500/30 group-hover:shadow-orange-500/5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          ...
          <div className="relative flex items-center justify-center min-h-[450px] group/viz">
            ...
            >
              ...
              <div className="relative z-10 flex flex-col items-center gap-4">
                {project.id === "news-nexus" && (
                  <>
                    <Newspaper className="w-20 h-20 text-slate-800 dark:text-white group-hover/viz:text-orange-500 transition-colors" strokeWidth={1} />
                    <span className="text-[10px] font-black uppercase tracking-[0.4em] text-orange-500">NEWS_ENGINE</span>
                  </>
                )}
                ...
                {project.id === "currency-converter" && (
                  <>
                    <RefreshCw className="w-20 h-20 text-orange-500/80 animate-spin-slow" strokeWidth={1} />
                    <span className="text-[10px] font-black uppercase tracking-[0.4em] text-orange-500">LIVE_EXCHANGE</span>
                  </>
                )}
                {project.id === "skycast" && (
                  <>
                    <CloudSun className="w-20 h-20 text-slate-800 dark:text-white group-hover/viz:text-orange-500 transition-colors" strokeWidth={1} />
                    <span className="text-[10px] font-black uppercase tracking-[0.4em] text-orange-500">SKY_INSIGHTS</span>
                  </>
                )}
              </div>
            ...
            {/* Floating Architecture Tags */}
            <div className="absolute inset-0 pointer-events-none">
              {project.id === "news-nexus" && (
                <>
                  <FloatingTag icon={<Database size={14}/>} text="Redis" delay={0} top="15%" left="15%" />
                  <FloatingTag icon={<Lock size={14}/>} text="Auth" delay={1} top="75%" left="12%" />
                  <FloatingTag icon={<Zap size={14}/>} text="OTP" delay={0.5} top="20%" right="15%" />
                </>
              )}
              {project.id === "smart-cart" && (
                <>
                  <FloatingTag icon={<ShieldCheck size={14}/>} text="RBAC" delay={0} top="15%" left="15%" />
                  <FloatingTag icon={<Activity size={14}/>} text="Analytics" delay={1} top="75%" left="12%" />
                  <FloatingTag icon={<Zap size={14}/>} text="Scalable" delay={0.5} top="20%" right="15%" />
                </>
              )}
            </div>
